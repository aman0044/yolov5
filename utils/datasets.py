...
#     a += random.choice([-180, -90, 0, 90])  # add 90deg rotations to small rotations # Aman : previous = commented
    s = random.uniform(1-scale, 1 + scale) # Aman : previous = (1 - scale, 1 + scale)
    ...
    if n:
        ...
        # filter candidates
        i = box_candidates(box1=targets[:, 1:5].T * s, box2=xy.T, wh_thr=3, ar_thr=30, area_thr=0.3)# Aman : previous default params
        targets = targets[i]
        targets[:, 1:5] = xy[i]
